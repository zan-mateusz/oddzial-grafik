...
def test_export_creates_readable_file(seeded, tmp_path):
    ...
    # Każde piętro trafia na osobną kartę skoroszytu.
    assert [g.name for g in grids] == [f["name"] for f in seeded.floors()]


def test_roundtrip_preserves_every_entry(seeded, tmp_path):
    """Eksport obu pięter i ponowny import muszą odtworzyć cały miesiąc."""
    original = seeded.month_entries(2026, 9)
    path = export_month(tmp_path / "g.xlsx", seeded, 2026, 9)

    grids = {g.name: g for g in xi.read_sheets(path)}
    floors = seeded.floors()
    assert set(grids) == {f["name"] for f in floors}

    target = Database(tmp_path / "target.db")
    for floor, new_floor in zip(floors, target.floors()):
        assert floor["name"] == new_floor["name"]
    ...
    imported = 0
    for floor in floors:
        grid = grids[floor["name"]]
        layout = xi.detect_layout(grid, calendar.monthrange(2026, 9)[1])
        assert layout.ok and len(layout.day_cols) == 30
        rows = xi.extract_rows(grid, layout)
        xi.match_employees(rows, target.employees())
        assert all(r.employee_id for r in rows), floor["name"]
        count, created = xi.apply_import(
            target, 2026, 9, rows, replace=False, floor_id=floor["id"]
        )
        assert created == 0
        imported += count
...
def test_roundtrip_keeps_each_shift_on_its_own_floor(seeded, tmp_path):
    """Zastępstwa nie mogą po imporcie wylądować na macierzystym piętrze."""
    path = export_month(tmp_path / "g.xlsx", seeded, 2026, 9)
    grids = {g.name: g for g in xi.read_sheets(path)}
    ...
    for emp in seeded.employees():
        target.add_employee(emp["last_name"], emp["first_name"],
                            floor_id=emp["floor_id"])
    for floor in seeded.floors():
        grid = grids[floor["name"]]
        rows = xi.extract_rows(grid, xi.detect_layout(grid, 30))
        xi.match_employees(rows, target.employees())
        xi.apply_import(target, 2026, 9, rows, replace=False, floor_id=floor["id"])

    for floor in seeded.floors():
        before = seeded.month_entries(2026, 9, floor["id"])
        after = target.month_entries(2026, 9, floor["id"])
        assert len(before) == len(after), floor["name"]
    target.close()
...
def test_import_skips_legend_and_footer_rows(seeded, tmp_path):
    ...
    path = export_month(tmp_path / "g.xlsx", seeded, 2026, 9)
    grids = xi.read_sheets(path)
    grid = grids[xi.best_sheet_index(grids, 30)]
    rows = xi.extract_rows(grid, xi.detect_layout(grid, 30))
